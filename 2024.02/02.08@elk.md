...
spec:
  ...
  type: LoadBalancer
```
- elsticsearcj config 파일에서 http.port를 선언하지 않아 외부로부터는 접근 불가능
...
```
https://www.elastic.co/guide/en/beats/metricbeat/7.17/running-on-kubernetes.html
- metricbeat
  ...
  template:
    ...
    spec:
      ...
      - name: metricbeat
        ...
        volumeMounts:
        - name: config
          mountPath: /usr/share/metricbeat/metricbeat.yml # 수정
          readOnly: true
...
k delete configmap metricbeat-daemonset-config -n kube-system
k delete configmap metricbeat-daemonset-modules -n kube-system
k delete pod metricbeat-4jjrt -n kube-system

k get pod -n kube-system -o wide
k exec -it metricbeat-4jjrt -n kube-system -- bash

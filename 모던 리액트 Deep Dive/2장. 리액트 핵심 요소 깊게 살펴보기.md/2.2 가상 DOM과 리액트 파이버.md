...
- 컴포넌트 호출을 통해 반환된 react element를 current fiber와 비교하여 workInProgress를 새로 만들어야 하는지, 아니라면 어떤 부분들이 변경되었는지, current는 삭제되어야 하는지 등 두 컴포넌트를 재조정한다.
